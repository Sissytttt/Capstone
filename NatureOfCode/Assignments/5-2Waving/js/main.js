...
function setupThree() {
  ...
  for (let i = 0; i < wave_num; i++) {
    Wvel = random(0.005, 0.03);
    Wlen = random(0.003, 0.009);
    Wamp = random(30, 60);
    noise_move = random(100);
    waves.push([Wvel, Wlen, Wamp, noise_move]);
  }
...
class Particle {
  ...
  setWave(Wvel, Wlen, Wamp, noise_move, WfreqAdj = 1, WampAdj = 1) {
  ...
  moveUp(spd) {
    ...
    let yFreq = this.pos.y * 0.05 + frame * 0.005 + 1000;
    let noise_moveup = map(noise(xFreq, yFreq), 0.0, 1.0, -0.25 * spd, spd);
