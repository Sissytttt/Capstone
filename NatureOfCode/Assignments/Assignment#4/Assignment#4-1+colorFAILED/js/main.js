...
function setupThree() {
  ...
  for (let i = 0; i < params.PARTICLE_NUMBER; i++) {
    ...
    let size = map(noiseValue, 0, 1, 1, 10);
...
function updateThree() {
  ...
  while (particles.length < params.PARTICLE_NUMBER) {
    ...
    let yPos = cos(random_angle) * WORLD_SIZE;
    let noiseValue = noise(noiseScale * xPos, noiseScale * yPos);
    let size = map(noiseValue, 0, 1, 10, 50);
    let brightness = map(noiseValue, 0, 1, 100, 255);
    let p = new Particle()
      .setPos(xPos, yPos, 0)
      .setSize(size)
  ...
  // let OpacityArray = pointCloud.geometry.attributes.opacity.array;
  let sizeArray = pointCloud.geometry.attributes.size.array;
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    // OpacityArray[ptIndex] = p.opacity;
    sizeArray[ptIndex] = p.size;
  }
...
function getPoints(objects) {
  ...
  const opacities = new Float32Array(objects.length);
